...
drug_result2<-drug_info[match(rownames(ic50),drug_info$drug_id),]
...
HDAC_ic50<-d_ic50[match(as.character(HDAC[,2]),rownames(d_ic50)),]
...
setwd("~/xjj/drug/drug_result/durg62/S8043_S1047/7_CNV/fisher")
write.table(amp_gene1,"sensitivity-resistance-amp-CNV-gene.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
write.table(censored_gene1,"sensitivity-resistance-censored-CNV-gene.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
######### CNV KEGG
library(org.Hs.eg.db)
library(clusterProfiler)
library(stringr)
gene=bitr(censored_gene1[,1],fromType="SYMBOL",toType="ENTREZID",OrgDb="org.Hs.eg.db") #会有部分基因数据丢失，或者ENSEMBL
...
G<-"OXA"
name<-"OXA"
up_name<-"P0.01_OXA_up"
down_name<-"P0.01_OXA_down"
file_name<-"OXA"
...
clusterProfiler<-function(file_name,name,keggfreq,kegg_freq_name,gofreq,go_freq_name){
  ...
  kegg<-enrichKEGG(gene[,2],organism = 'hsa',keyType = 'kegg',pvalueCutoff = 0.05,pAdjustMethod = 'none',
                              minGSSize = 10,maxGSSize = 500,qvalueCutoff = 1,use_internal_data = FALSE)
  #kegg<-data.frame(enrichKEGG(gene[,2],organism = 'hsa',keyType = 'kegg',pvalueCutoff = 0.05,pAdjustMethod = 'none',
  #               minGSSize = 10,maxGSSize = 500,qvalueCutoff = 1,use_internal_data = FALSE))
  ke<-barplot(kegg,showCategory=10,drop=T,x = "GeneRatio",color = "p.adjust")
  #ke<-barplot(kegg,showCategory=16,drop=T,x = "GeneRatio",color = "p.adjust")
  plot(ke)
  ee<-kegg@result
...
#res<-read.table(paste("sensitivity-resistance-all-DESeq2_",file_name,"_ATAC.txt",sep=""),sep = '\t',header = T)##数据输出
resSig<-res[which(res$pvalue<freq),]
# pvalue padj
resSig[which(resSig$log2FoldChange>0),'up_down']<-'up'
resSig[which(resSig$log2FoldChange<0),'up_down']<-'down'
...
library(VennDiagram)
file_name<-"OXA"
anno_name_down<-"OXA-0.05-P-down" #注释文件
anno_name_up<-"OXA-0.05-P-up"
up_name<-"P0.05_OXA_up"  #差异基因文件
down_name<-"P0.05_OXA_down"
...
file_name<-"5-FU"
anno_name_down<-"5-FU-0.05-P-up" #注释文件
down_name<-"P0.05_5-FU_up"
freq_name<-"P0.05_up"
...
rm(list=ls())
TF_names<-"sen-res-5-FU-0.001-P-logFC0-DESeq2-up"
file_name<-"5-FU"
...
library(stringr)
file_name<-"OXA"
DE_gene<-"P0.05_OXA_down" #or down
TF_names<-"sen-res-OXA-0.05-P-logFC0-DESeq2-down"
freq<-0.05  #FDR<0.05
venn_name<-"down_DETFs_P005"
TF_target_name<-"TRANSFAC" #JASPAR, MotifMap, ENCODE
up_name<-"P0.05_OXA_up"
down_name<-"P0.05_OXA_down"
DE_name<-"P0.05_OXA_down"
regulation<-"down"
...
DETF_Statistics4<-data.frame(matrix(0,nrow(DETFs),3))
for(k in 1:nrow(DETFs)){
  file_name<-as.character(DETFs[k,1])
  DETF_Statistics4[k,1]<-file_name
  ...
  colnames(target_TF_gene)<-c("Target","TFs")
  DETFs_up<-unlist(strsplit(as.character(DETFs[k,3]),','))#3 up 5 down
  ...
  if(length(unique(result[,2]))>0){
    ...
    cat(paste("The DETFs contain target is ",unique(data[,1]),sep=""),"\n")
    setwd(paste("~/xjj/drug/drug_result/durg62/",file_name,"/4_TFs/DETFs-target-DEGs",sep=""))
    write.table(data,paste("P0.05_up_DEGs_DApeaks_FDR",freq,"_DETFs_",TF_target_name,"-gene.txt",sep=""),sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
    ...
    DETF_Statistics4[k,2]<-paste0(unique(result[,2]),collapse = ",")
    DETF_Statistics4[k,3]<-paste0(jresult,collapse =",")
  }
  
  if(length(unique(result[,2]))==0){
    DETF_Statistics4[k,2]<-0
    DETF_Statistics4[k,3]<-0
  }
}
colnames(DETF_Statistics4)<-c(paste("drug",TF_target_name,"up",sep=" "),paste("DETF",TF_target_name,"up",sep=" "),paste("Number of DETF",TF_target_name,"up",sep=" "))
...
#}
setwd("~/xjj/drug/drug_result/durg62")
write.table(result9,"DETF_Statistics_42drug_all.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
