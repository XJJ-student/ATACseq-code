...
library(pheatmap)
a=cor(t(HDAC_ic50))
result=pheatmap(a,scale = "none",main = "The correlation coefficients between HDACi' AUC",show_rownames=T,show_colnames=T,
                clustering_distance_rows = "correlation",
                clustering_distance_cols = "correlation",clustering_method = "complete",cutree_rows=2,cutree_cols=2)

###########  HDACi20种药物过滤不相关的2个药物
clusters<-cutree(result$tree_row,k=2)
#a=table(clusters)
ic50_filter=HDAC_ic50[clusters==2,]
...
norma_result<-pheatmap(bb,scale = "none",main = "The AUC of HDACi is normalized",show_rownames=T,show_colnames=T,
                       clustering_distance_rows = "euclidean",
                       clustering_distance_cols = "euclidean",clustering_method = "ward.D2")
...
exp<-exp2[(-cou),]
label=c(rep(1,length(sensitivity)),rep(2,length(medianl)),rep(3,length(resistance1)))
setwd("~/xjj/drug")
sample_id<-read.csv("sample_id.csv",header = TRUE,sep = ",")
resistance_new<-sample_id[match(resistance1,sample_id[,2]),1]
sensitivity_new<-sample_id[match(sensitivity,sample_id[,2]),1]
median_new<-sample_id[match(medianl,sample_id[,2]),1]
resistance_exp<-exp[,match(resistance_new,colnames(exp))]
sensitivity_exp<-exp[,match(sensitivity_new,colnames(exp))]
median_exp<-exp[,match(median_new,colnames(exp))]
cbind_exp<-cbind(sensitivity_exp,median_exp,resistance_exp)
geneid<-exp1[(-cou),1]
library(foreach)
library(broom)

anova_result<-data.frame(matrix(0,nrow(cbind_exp),3))
foreach(k=1:nrow(cbind_exp),.combine=rbind)%do%{
  anova_result[k,1]<-geneid[k]
  cat(k,"\n")
  temp1=data.frame(label,value=t(cbind_exp[k,]))
  colnames(temp1)<-c("label","value")
  ano<-anova(lm(temp1$value~temp1$label))
  anova_result[k,2]<-tidy(ano)$p.value[1]
  anova_result[k,3]<-tidy(ano)$statistic[1]
}
colnames(anova_result)<-c("geneid","pvalue","Fscore")
anova_result[,4]<-p.adjust(anova_result[,2],method="BH")
DEGs<-anova_result[which(anova_result[,2]<0.05),]

setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/3_RNAseq_anova")
write.table(anova_result,"sensitivity-resistance-all_HDAC_AUC_RNAseq_DEGs_anova.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
#resSig<-res[which(res$pvalue<0.05 & abs(res$log2FoldChange>1)),]
resSig<-anova_result[which(anova_result[,2]<0.05),]
##新增一列，将log2FoldChange>0标注为up，<0标准为down
write.table(resSig,"sensitivity-resistance-HDAC_AUC_RNAseq_DEGs_P0.05_anova.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出

############  画热图查看差异基因在三类样本中的表达情况
rm(list=ls())
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/3_RNAseq_anova")
DEG<-read.table("sensitivity-resistance-all_HDAC_AUC_RNAseq_DEGs_anova.txt",sep = '\t',header=T)##数据输出
anova_DEG1<-DEG[which(DEG[,2]<0.01),]
anova_DEG<-anova_DEG1[order(anova_DEG1[,2]),]
setwd("~/176/Changhai_mRNA/2_Expression/Summary/star_rsem")
library("data.table")
exp1<-fread("star_rsem.GeneSymbol.TPM.xls",header=T,data.table=F)
exp2<-exp1[,-1]
...
geneid<-exp1[(-cou),1]
cbind_exp<-cbind(sensitivity_exp,median_exp,resistance_exp)
rownames(cbind_exp)<-geneid

DEG_exp<-cbind_exp[match(anova_DEG[,1],geneid),]
pheatmap(DEG_exp[1:20,],scale = "none",main = "The DEGs of three types of samples",show_rownames=T,show_colnames=T,
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_RNAseq")
write.table(res,"sensitivity-resistance-all-DESeq2_HDAC_AUC_RNAseq_DEGs.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
#resSig<-res[which(res$pvalue<0.05 & abs(res$log2FoldChange>1)),]
...
library(ggplot2)
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_RNAseq")
df<-read.table("sensitivity-resistance-all-DESeq2_HDAC_AUC_RNAseq_DEGs.txt",sep = '\t',header= T,row.names = 1)
#确定是上调还是下调，用于给图中点上色
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_RNAseq/Acetylase")
...
barplot(kegg,showCategory=10,drop=T,x = "GeneRatio",color = "p.adjust")
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_RNAseq/pathway")
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC/1_RNAseq_DEGs/pathway")
...
library("data.table")
peak_rpkm_name<-fread("peak_RPKM.txt",header=T,data.table=F)
...
gsub("\\.","-",colnames(peak_rpkm_name))
resistance_peak<-peak_rpkm_name[,match(resistance_new,gsub("\\.","-",colnames(peak_rpkm_name)))]
sensitivity_peak<-peak_rpkm_name[,match(sensitivity_new,gsub("\\.","-",colnames(peak_rpkm_name)))]
median_peak<-peak_rpkm_name[,match(median_new,gsub("\\.","-",colnames(peak_rpkm_name)))]
peak_name<-peak_rpkm_name[,1]
cbind_exp<-cbind(sensitivity_peak,median_peak,resistance_peak)
library(foreach)
library(broom)

anova_result<-data.frame(matrix(0,nrow(cbind_exp),3))
foreach(k=1:nrow(cbind_exp),.combine=rbind)%do%{
  anova_result[k,1]<-peak_name[k]
  cat(k,"\n")
  temp1=data.frame(label,value=t(cbind_exp[k,]))
  colnames(temp1)<-c("label","value")
  ano<-anova(lm(temp1$value~temp1$label))
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/4_ATACseq_anova")
write.table(anova_result,"sensitivity-resistance-all_HDAC_AUC_ATAC_anova.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
#resSig<-res[which(res$pvalue<0.05 & abs(res$log2FoldChange>1)),]
resSig<-anova_result[which(anova_result[,2]<0.05),]
##新增一列，将log2FoldChange>0标注为up，<0标准为down
write.table(resSig,"sensitivity-resistance-HDAC_AUC_RNAseq_DEGs_P0.05_anova.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
...
#res<-read.table("sensitivity-resistance-all-DESeq2_HDAC_AUC_ATAC.txt",header = TRUE,sep = "\t")
...
out_file<-"sen-res-HDAC_AUC-0.001-P"
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/2_ATACseq_DApeaks")
library(data.table)
homer_anno<-fread("sen-res-HDAC_AUC-0.001-P-logFC0-DESeq2-up-annotation.txt",header=T,data.table=F)
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/2_ATACseq_DApeaks")
library(data.table)
homer_anno_down<-fread("sen-res-HDAC_AUC-0.05-P-logFC0-DESeq2-down-annotation.txt",header=T,data.table=F)
...
homer_anno_up<-fread("sen-res-HDAC_AUC-0.05-P-logFC0-DESeq2-up-annotation.txt",header=T,data.table=F)
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_2_intersect/venn")
library(VennDiagram)
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/2_ATACseq_DApeaks")
DApeak<-fread("sensitivity-resistance-all-DESeq2_HDAC_AUC_ATAC.txt",header=T,data.table=F)
...
pathway_gene<-unique(unlist(strsplit(enrich_gene,split="/")))
pathway_gene2=bitr(pathway_gene,fromType="ENTREZID",toType="SYMBOL",OrgDb="org.Hs.eg.db") #会有部分基因数据丢失，或者ENSEMBL
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC_correlation/1_2_intersect/pathway")
write.table(ee1,"DEGP0.05_kegg_pathway_DEGs_DApeaks_FDR0.05_down.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
write.table(pathway_gene,"DEGP0.05_kegg_pathway_DEGs_DApeaks_FDR0.05_down_gene.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
...
rm(list=ls())
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC/1_RNAseq_DEGs")
up_gene<-read.table("sensitivity-resistance-all-DESeq2_AUC_RNAseq_P0.05_up_DEGs.txt",header=T,sep="\t")

setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC/2_ATACseq_DApeaks/homer_result/sen-res-HDAC_AUC-0.05-P-logFC0-DESeq2-up")
knownResults<-read.delim("knownResults.txt",header=T,sep='\t')
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC/1_RNAseq_DEGs")
up_gene<-read.table("sensitivity-resistance-all-DESeq2_AUC_RNAseq_P0.05_up_DEGs.txt",header=T,sep="\t")
down_gene<-read.table("sensitivity-resistance-all-DESeq2_AUC_RNAseq_P0.05_down_DEGs.txt",header=T,sep="\t")
...
setwd("~/xjj/drug/drug_result/HDAC_frontiers_AUC/4_TF/TRANSFAC_TF/DETFs-target-DEGs")
write.table(data,"P0.05_down_DEGs_DApeaks_FDR0.05_DETFs_TRANSFAC-gene.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
write.table(data,"P0.05_up_DEGs_DApeaks_FDR0.05_DETFs_JASPAR-gene.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
