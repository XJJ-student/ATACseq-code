...
GEM_sensitivity<-colnames(want_AUC_chemo)[which(want_AUC_chemo>0.5)]
GEM_resistance<-colnames(want_AUC_chemo)[which(want_AUC_chemo<0.5)]
S8495_sensitivity<-colnames(want_AUC_HDAC)[which(want_AUC_HDAC>0.5)]
S8495_resistance<-colnames(want_AUC_HDAC)[which(want_AUC_HDAC<0.5)]
...
write.table(res,"sensitivity-resistance-all-DESeq2_S8495_AUC_RNAseq_DEGs.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_RNAseq")
write.table(up_gene,"sensitivity-resistance-S8495-DESeq2_AUC_RNAseq_P0.05_FC0_up_DEGs.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
write.table(down_gene,"sensitivity-resistance-S8495-DESeq2_AUC_RNAseq_P0.05_FC0_down_DEGs.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
...
library(ggplot2)
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_RNAseq")
df<-read.table("sensitivity-resistance-all-DESeq2_S8495_AUC_RNAseq_DEGs.txt",sep = '\t',header= T,row.names = 1)
#确定是上调还是下调，用于给图中点上色
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_RNAseq/Volcano")
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_RNAseq/Acetylase")
...
sensitivity<-c("DAC-23","DAC-26","DAC-31","DAC-7","DAC-8","DAC-2","DAC-14")
resistance<-c("DAC-21","DAC-22","DAC-25","DAC-30","DAC-35","DAC-13")
...
write.table(res,"sensitivity-resistance-all-DESeq2_S8495_AUC_ATAC.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)##数据输出
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq")
out_file<-"sensitivity-resistance-S8495-AUC-P0.05-logFC1"
...
library(data.table)
homer_anno<-fread("sensitivity-resistance-S8495-AUC-P0.01-logFC0-DESeq2-down-annotation.txt",header=T,data.table=F)
...
p = ggplot(dt, aes(x = "", y = A,fill = B)) + #创建坐标轴，fill = B
  ...
  labs(title = "up-peaks-p0.01")+
  theme(plot.title = element_text(hjust = 0.5))
#geom_text(aes(y = A/2 + c(0, cumsum(A)[-length(A)]), x = sum(A)/20, label = myLabel), size = 5)   # 在图中加上百分比：x 调节标签到圆心的距离, y 调节标签的左右位置
print(p) #显示饼图
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/annotation")
ggsave("up_P0.01_FC0_annotation.pdf",p,width = 6, height = 5)
write.table(result,file="up_P0.01_FC0_annotation_ratio.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE)
...
homer_anno1<-fread("sensitivity-resistance-S8495-AUC-P0.01-logFC0-DESeq2-down-annotation.txt",header=T,data.table=F)
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq")
res<-read.table("sensitivity-resistance-all-DESeq2_S8495_AUC_ATAC.txt",header = TRUE,sep = "\t")
resSig<-res[which(res$pvalue<0.01),]
...
p = ggplot(dt, aes(x = "", y = A, fill = B)) + #创建坐标轴
  ...
  geom_bar(stat = "identity", width = 1) +   #当width >= 1 时中心的杂点将消失
  coord_polar(theta = "y") +  # 把柱状图折叠成饼图（极坐标）
  labs(x = "", y = "", title = "allpeaks-position") +  # 将横纵坐标的标签设为空
  theme(axis.ticks = element_blank()) +  # 将左上角边框的刻度去掉
  theme(legend.title = element_blank(), legend.position = "left")+   ## 将图例标题设为空，并把图例方放在左边位置
  scale_fill_discrete(breaks = dt$B, labels = myLabel)+   # 将原来的图例标签换成现在的myLabel
  theme(axis.text.x = element_blank())+   ## 去掉饼图的外框上的数值，即去除原柱状图的X轴，把X轴的刻度文字去掉
...
print(p) #显示饼图
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/annotation")
ggsave("downpeaks-P0.01-position.pdf",p,width = 6, height = 5)
write.table(cbind(data,dt),file="downpeaks-P0.01-position.txt",sep = '\t',col.names = T,row.names = F,quote = FALSE,na='')
...
colnames(dat_m) = c('Group','Type','value')
#定义`Group`列的出图顺序
dat_m$Group = factor(dat_m$Group, levels = c("allpeaks-peaks","downpeaks-peaks","uppeaks-peaks"))
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/annotation")
library(data.table)
homer_anno_down<-fread("sensitivity-resistance-S8495-AUC-P0.01-logFC0-DESeq2-down-annotation.txt",header=T,data.table=F)
...
down_gene<-read.table("sensitivity-resistance-S8495-DESeq2_AUC_RNAseq_P0.05_FC0_down_DEGs.txt",header=T,sep="\t")
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_2_intersect/venn")
library(VennDiagram)
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_2_intersect/correlation")
...
plot(ps)
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_2_intersect/correlation")
ggsave("correlation-DApeaksP0.01-DEGsP0.05-pearson-S8495-onlyone.pdf",ps,width = 10, height = 10)
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_2_intersect/pathway")
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/FIMO/P0.01_FC0_down")
...
DApeak<-fread("sensitivity-resistance-all-DESeq2_S8495_AUC_ATAC.txt",header=T,data.table=F)
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/1_RNAseq")
DE_gene<-fread("sensitivity-resistance-all-DESeq2_S8495_AUC_RNAseq_DEGs.txt",header=T,data.table=F)
...
#######
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/FIMO/P0.01_FC0_up")
...
pu<-left_1+right_2+labs(title="Regression coefficients showing enrichmen of TFs")+
  theme(plot.title = element_text(hjust = 0.5))
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/FIMO")
...
setwd("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/TF_target")
...
### ATAC
library("data.table")
df1<-read.table("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/sensitivity-resistance-all-DESeq2_S8495_AUC_ATAC.txt",sep = '\t',header= T)
...
df11<-read.table("~/xjj/drug/drug_result/HDACi_chemo618/sen_or_res_to_S8495/2_ATACseq/sensitivity-resistance-all-DESeq2_S8495_AUC_ATAC.txt",sep = '\t',header= T)
